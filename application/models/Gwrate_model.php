<?php
class Gwrate_model extends CI_Model {
    ...
    public function search($tariffname, $prefix, $tariffdesc){
        ...
        if(($first_character == '%') && ($last_character == '%')){
            ...
            $this->db->like('tariffdesc', $key);
        } else if (($first_character == "%") && ($last_character != '%')){
            $key = substr($tariffdesc, 1, strlen($tariffdesc) - 1);
            $this->db->like('tariffdesc', $key, 'before');
        } else if (($first_character != "%") && ($last_character == '%')){
            $key = substr($tariffdesc, 0, strlen($tariffdesc) - 1);
            $this->db->like('tariffdesc', $key, 'after');
        } else {
            $this->db->where('tariffdesc', $tariffdesc);
        }
    ...
    public function add_gwrate($data){
    	$this->db->insert('add_gw_rates', $data);
    ...
    public function update_gwrate($data, $id){
        $condition = array(
            'id' => $id
        );
        $this->db->set($data);
        $this->db->where($condition);
        $this->db->update('add_gw_rates');
